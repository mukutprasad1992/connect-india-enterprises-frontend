...
const MutualFundFormDialog: React.FC<Props> = ({
  ...
  const [fileToRemove, setFileToRemove] = useState<keyof MutualFundData | null>(
    null,
  );
    ...
    (
      ...
      switch (name) {
        ...
        case "nomineeMobile":
          ...
          if (typeof value === "string") {
            const cleanNomineeMobile = value
              .replace(/^(\+91)/, "")
              .trim()
              .replace(/\D/g, "");
        ...
        case "placeOfBirth":
          ...
          if (fullName.length > 200)
            return "Place of birth cannot exceed 200 characters";
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      name === "panNumber" ||
      (name === "nomineeId" && formData.nomineeIdType === "pan")
    ) {
      ...
      const maxLength =
        name === "aadharNumber" || formData.nomineeIdType === "aadhar"
          ? 16
          : 12;
  ...
  const handleSelectChange = (event: SelectChangeEvent<string>) => {
    ...
    setFormData((prev) => {
      ...
      if (prev.occupation === "Job" && value !== "Job") {
        setFileCache((cache) => ({
          ...cache,
          salarySlips: prev.salarySlipsFileKey,
        }));
      ...
      if (prev.occupation === "Business" && value !== "Business") {
        setFileCache((cache) => ({
          ...cache,
          itrDocuments: prev.itrDocumentsFileKey,
        }));
      ...
      if (
        value === "Business" &&
        prev.occupation !== "Business" &&
        fileCache.itrDocuments
      ) {
      ...
      switch (stepKey) {
        ...
        case "documents":
          stepData = {
            ...
            bankProofFileKey: updatedFormData.bankProofFileKey,
            salarySlipsFileKey: updatedFormData.salarySlipsFileKey,
            itrDocumentsFileKey: updatedFormData.itrDocumentsFileKey,
          };
          break;
  ...
  const validateStep = useCallback(() => {
    ...
    switch (step) {
      case 1:
        ["aadharNumber", "panNumber"].forEach((field) => {
          const error = validateField(
            field,
            formData[field as keyof MutualFundData] as string | File | null,
          );
          if (error) {
        ...
        break;
      case 2:
        ["email", "mobile", "placeOfBirth", "income", "occupation"].forEach(
          (field) => {
            const error = validateField(
              field,
              formData[field as keyof MutualFundData] as string | File | null,
      ...
      case 3:
        ...
        break;
      case 4:
        const documentFields: (keyof MutualFundData)[] = [
          "aadharCardFileKey",
          "panCardFileKey",
          "bankProofFileKey",
        ];
        if (formData.occupation === "Job")
          documentFields.push("salarySlipsFileKey");
        if (formData.occupation === "Business")
          documentFields.push("itrDocumentsFileKey");
            ...
            <Grid item {...gridSpacing}>
              <TextField
                ...
                inputProps={{
                  ...
                  style: {
                    textTransform:
                      formData?.nomineeIdType === "pan" ? "uppercase" : "none",
                  },
                }}
                onInput={(e: React.FormEvent<HTMLInputElement>) => {
                  const input = e.currentTarget;
                ...
                helperText={
                  errors.nomineeId || formData.nomineeIdType === "aadhar"
                }
                disabled={loading}
                className="customTextField"
              />
              ...
              <TextField
                ...
                value={
                  formData?.nomineeMobile
                    ? formData.nomineeMobile.replace(/^(\+91 )/, "")
                    : ""
                }
                onChange={(e) => {
                  setFormData({
        ...
        <DialogTitle>
          ...
          >
            <Typography variant="h6">
              {mode === "edit"
                ? "Edit mutual fund or SIP"
                : "New mutual fund or SIP"}
            </Typography>
          ...
          {success ? (
            <Box sx={{ textAlign: "center", p: 4 }}>
              ...
              <Typography color="text.secondary">
                Your mutual fund application has been received. Well contact you
                shortly.
              </Typography>
            </Box>
          ) : (
