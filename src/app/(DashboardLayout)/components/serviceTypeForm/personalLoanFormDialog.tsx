...
const PersonalLoanFormDialog: React.FC<Props> = ({
  ...
  const [dialogFileType, setDialogFileType] = useState<"image" | "pdf" | null>(
    null,
  );
  ...
  const [declarationIsDetailsConfirmed, setDeclarationIsDetailsConfirmed] =
    useState(false);
  ...
  const [fileToRemove, setFileToRemove] = useState<keyof LoanFormData | null>(
    null,
  );
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "panNumber") {
      ...
      const maxLength = name === "aadharNumber" ? 16 : 12;
  ...
  const validateField = useCallback(
    (name: keyof LoanFormData, value: string | File | null): string => {
      switch (name) {
        ...
        case "ref2Mobile":
          ...
          if (!/^[6-9]\d{9}$/.test(value))
            return "Invalid 10-digit mobile number (must start with 6-9)";
          return "";

        case "panNumber":
          if (typeof value !== "string") return "Invalid PAN input";
          const upperValue = value.toUpperCase();
          if (!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(upperValue))
        ...
        case "maritalStatus":
          ...
          if (!maritalStatusOptions.includes(value))
        ...
        case "designation":
          ...
          if (value.length > 50)
            return "Designation cannot exceed 50 characters";
          return "";
        ...
        case "totalWorkExp": {
          ...
          if (numValue.toString().length > 2)
            return "Must be a number (max 2 digits)";
    ...
    const validateFields = (fields: Array<keyof LoanFormData>) => {
      fields.forEach((field) => {
        const error = validateField(
          field,
          formData[field] as string | File | null,
        );
        if (error) {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (step === 5) {
        ...
        try {
          const uploadPromises = fileKeys.map(async ({ field, folder }) => {
            const file = formData[field as keyof LoanFormData];
          ...
          results.forEach((res) => {
            if (res) {
              updatedFormData[res.field] = res.key;
            }
          });
          setFormData(formData);
          await saveStepData(step, updatedFormData);
          setInitialFormData(formData);
          setStep((prev) => prev + 1);
          setMaxAllowedStep(Math.max(maxAllowedStep, step + 1));
        } catch (err: any) {
          setErrors((prev) => ({
            ...prev,
            form: `Failed to upload ${err.message}. Please try again.`,
          }));
          return;
        }
      } else {
  ...
  const saveStepData = async (stepNumber: number, updatedFormData: any) => {
    try {
      const token = getToken();
      if (!token) return;
      ...
      switch (stepKey) {
        ...
        case "documents":
          stepData = {
            aadharCardFileKey: updatedFormData.aadharCardFileKey,
            panCardFileKey: updatedFormData.panCardFileKey,
            photoFileKey: updatedFormData.photoFileKey,
            salarySlipsFileKey: updatedFormData.salarySlipsFileKey,
            bankStatementFileKey: updatedFormData.bankStatementFileKey,
          };
          break;
      ...
      if (formData.id) {
        ...
        const res = await axios.post(`${BASE_URL}/loan/createLoan`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setSnackbarSeverity("success");
        setSnackbarOpen(true);
  ...
  const handleFormSubmit = async () => {
    if (!formData.id && !serviceId) {
      ...
      setErrors((prev) => ({
        ...prev,
        form: "Service ID missing. Please start over.",
      }));
      return;
    }
          ...
          <FormControl fullWidth error={!!error}>
            ...
            >
              <Grid container alignItems="center" spacing={1}>
                ...
                <Grid item xs={12} sm={3}>
                  <Typography
                    variant="body1"
                    sx={{ fontWeight: 600, ml: 3, fontSize: 11 }}
                  >
                ...
                <Grid item xs={12} sm={2.5}>
                  ...
                  >
                    {fileValue || filePreviewUrls[name]
                      ? "File Uploaded"
                      : "Choose File"}
                    <input
                      hidden
                      ...
                      disabled={
                        !!fileValue || !!filePreviewUrls[name] || loading
                      }
                    />
                  </Button>
                </Grid>
            ...
            {error && (
              <Typography
                variant="caption"
                color="error"
                sx={{ mt: 0.1, ml: 0.7, display: "block" }}
              >
                {error}
              </Typography>
            )}
  ...
  const renderReviewStep = () => (
    <Box sx={{ p: 2 }}>
      ...
      <Typography variant="h6" gutterBottom sx={{ mt: 1, fontSize: 14 }}>
        Personal Information
      </Typography>
      <Divider sx={{ mb: 3 }} />
      ...
      <Divider sx={{ mb: 3 }} />
      <Grid container spacing={2}>
        {formData.aadharCardFileKey &&
          renderReviewItem("Aadhar Card", formData.aadharCardFileKey)}
        {formData.panCardFileKey &&
          renderReviewItem("PAN Card", formData.panCardFileKey)}
        {formData.bankStatementFileKey &&
          renderReviewItem("Bank Proof", formData.bankStatementFileKey)}
        {formData.salarySlipsFileKey &&
          renderReviewItem("Salary Proof", formData.salarySlipsFileKey)}
        {formData.photoFileKey &&
          renderReviewItem("photo", formData.photoFileKey)}
      </Grid>
      <Box sx={{ mt: 4 }}>
        <Typography
          variant="subtitle1"
          ...
          <Grid container spacing={2}>
            <Grid item {...gridSpacing}>
              <TextField
                ...
                className="customTextField"
                value={
                  formData.yearsOfCity !== null ? formData.yearsOfCity : ""
                }
                onChange={(e) => {
          ...
          <Grid container spacing={2}>
            ...
            <Grid item {...gridSpacing}>
              <TextField
                ...
                label={
                  <span>
                    Office Mobile <span style={{ color: "red" }}>*</span>
                  </span>
                }
                name="officeMobile"
          ...
          <Grid container spacing={2}>
            ...
            <Grid item {...gridSpacing}>
              <TextField
                ...
                label={
                  <span>
                    Reference 1 Mobile <span style={{ color: "red" }}>*</span>
                  </span>
                }
                name="ref1Mobile"
            ...
            <Grid item xs={12}>
              <TextField
                ...
                label={
                  <span>
                    Reference 1 Address <span style={{ color: "red" }}>*</span>
                  </span>
                }
                name="ref1Address"
            ...
            <Grid item {...gridSpacing}>
              <TextField
                ...
                label={
                  <span>
                    Reference 2 Name <span style={{ color: "red" }}>*</span>
                  </span>
                }
                name="ref2Name"
            ...
            <Grid item {...gridSpacing}>
              <TextField
                ...
                label={
                  <span>
                    Reference 2 Mobile <span style={{ color: "red" }}>*</span>
                  </span>
                }
                name="ref2Mobile"
            ...
            <Grid item xs={12}>
              <TextField
                ...
                label={
                  <span>
                    Reference 2 Address <span style={{ color: "red" }}>*</span>
                  </span>
                }
                name="ref2Address"
                ...
                <Step key={label} completed={completed}>
                  <StepButton
                    ...
                    sx={{
                      ...
                      "& .MuiStepLabel-label": {
                        color:
                          stepNumber === maxAllowedStep && !completed
                            ? "orange"
                            : "grey",
                        fontWeight: "bold",
                      },
          ...
          {success ? (
            <Box sx={{ textAlign: "center", p: 4 }}>
              ...
              <Typography color="text.secondary">
                Your Loan application has been received. Well contact you
                shortly.
              </Typography>
            </Box>
          ) : (
