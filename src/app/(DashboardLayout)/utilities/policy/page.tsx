...
const pageName = "policyPage";

const Policy = () => {
  const [policies, setPolicies] = useState<any>(null);
  const [selectedId, setSelectedId] = useState("");
  const [selectedOption, setSelectedOption] = useState("");
  const [openDeletePoliciesDialog, setOpenDeletePoliciesDialog] = useState(false);
  ...
  const [openPolicyFormDialog, setOpenPolicyFormDialog] = useState(false);
  ...
  const deletePolicy = async () => {
    ...
    try {
      ...
      setDialogLoading(false)
      setPolicyErrorMessage(error.response?.data?.message || "Error deleting policy")
      console.error("Error during policy deletion:", error);
    }

    setOpenDeletePoliciesDialog(false);
  };
  ...
  const handleDeleteButton = (id: any) => {
    setSelectedId(id);
    setOpenDeletePoliciesDialog(true);
  };
  ...
  const exportToPDF = async () => {
    ...
    doc.setProperties({
      title: 'Policy List Report',
      subject: 'Policy Data Export',
      author: 'Your Application Name',
      keywords: 'policy, report, data',
      creator: 'Your Application Name'
    });

    doc.setFontSize(12);
    doc.text("Policy LIST", 14, 20);
    const logoWidth = 60;
    const logoHeight = 15;
    ...
    const bodyData = (policies || []).map((row: any) => [
      row.id || 'N/A',
      row.email || 'N/A',
    ...
    const fileName = `Policy_Report_${new Date().toISOString().slice(0, 10)}.pdf`;
    doc.save(fileName);
  ...
  const exportToExcel = () => {
    ...
    const headerKeys = Object.keys((policies && policies[0]) || {});
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "polices");
  ...
  return (
    <>
      ...
      <Dialog
        open={openDeletePoliciesDialog}
        onClose={() => setOpenDeletePoliciesDialog(false)}
        maxWidth="xs"
        fullWidth
      >
        {dialogLoading && (
        ...
        <DialogActions>
          <Button
            onClick={() => setOpenDeletePoliciesDialog(false)}
            variant="outlined"
          >
            Cancel
          </Button>
          <Button onClick={deletePolicy} variant="contained" color="primary">
            Delete
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogContent>
          ...
          >
            ...
            >
              <MenuItem value="lifeInsurance">Life Insurance Policy</MenuItem>
            </Select>
          </FormControl>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseAddPolicyDialog} variant="outlined">
            Cancel
          </Button>
          <Button
            variant="contained"
            onClick={() => {
              if (!selectedOption) {
                ...
                setSnackbarMessage("Please select an policies type");
                return;
              }
              setOpenPolicyFormDialog(false);
              setOpenDialog(true);
